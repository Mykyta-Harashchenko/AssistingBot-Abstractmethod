...
class WebMessanger(Messanger):
    def send_to_user(self, message: str) -> None:
        print(f'Fake output to web {message}')
...
def main():
    ...
    user_question = input('Choose interface: 1 = Terminal, 2 = Web: ')
    if user_question == '1':
        messanger = TerminalMessanger()
    elif user_question == '2':
        messanger = WebMessanger()
    else:
    ...
    while True:
        ...
        if command in ["close", "exit"]:
            messanger.send_to_user("Good bye!")
            break

        elif command == "hello":
            messanger.send_to_user("How can I help you?")

        elif command == "add":
            messanger.send_to_user(add_contact(args, book))

        elif command == "change":
            messanger.send_to_user(change_contact(args, book))

        elif command == "phone":
            messanger.send_to_user(show_phones(args, book))

        elif command == "all":
            messanger.send_to_user(show_all(book))

        elif command == "add-birthday":
            messanger.send_to_user(add_birthday(args, book))

        elif command == "show-birthday":
            messanger.send_to_user(show_birthday(args, book))

        elif command == "birthdays":
            messanger.send_to_user(birthdays(args, book))

        else:
            messanger.send_to_user("Invalid command.")
